...
'******************************************************************************************************************
'   'Module  version:  1.4.0.2  (updated  on  2024.05.08)
'
'
' Author  list:
'           VBDX9BAS.BAS   Autumn  maple  rustles (GITHUB  https://github.com/CoolWindValley)
'                                      (Baidu  Tieba  VB  Bar  https://tieba.baidu.com/f?kw=vb)
'                         YY  bacteria  (yjryym)  (Baidu  Tieba  VB  Bar  https://tieba.baidu.com/f?kw=vb)
'           ACTMVX.TLB:   YY  bacteria  (yjryym)  (Baidu  Tieba  VB  Bar  https://tieba.baidu.com/f?kw=vb)
'           DX9VB.TLB     acme_pjz    GITHUB https://github.com/acmepjz
'           Engine  Icon:  jin          (Lanza  https://lanzainc.xyz/  https://indienova.com/u/lanza)
'
' If  you  have  any  comments,  suggestions  or  questions,  you  can  go  to  Baidu  Post  Bar  VB  Bar  to  post  a  discussion
' Or  submit  issues  on  the  GITHUB  repository  (https://github.com/CoolWindValley/CoolWind2D-GameEngine-CHS)!
'
'******************************************************************************************************************
...
Public Type POINTAPI    'API mouse position class
   X As Long
   Y As Long
End Type

Public Enum JOYFALGS    'API game handle class
    JOY_RETURNX = &H1
    JOY_RETURNY = &H2
    JOY_RETURNZ = &H4
    JOY_RETURNR = &H8
...
Public Enum JOYBUTTON    'API game handle class
    JOY_BUTTON1 = &H1
    JOY_BUTTON2 = &H2
    JOY_BUTTON3 = &H4
    JOY_BUTTON4 = &H8
...
Public Type JOYINFOEX    'API game handle class
   dwSize As Long
   dwFlags As JOYFALGS
   dwXpos As Long
   dwYpos As Long
...
Public Type Rect    'Long integer rectangle class
    Left As Long
    Top As Long
    Right As Long
    Bottom As Long
...
Public Type GpRect    'GDI+ rectangle class
    X As Long
    Y As Long
    Width As Long
    Height As Long
...
Public Type D2DVector   '2D vertex class
    X As Single
    Y As Single
    Z As Single
    Rhw As Single
...
Public Type CWPic       'Pictures
    Tex As Direct3DTexture9
    PICSize As D3DRECT
End Type

Public Type CWFont      'Font class
    SNum As Long
End Type

Public Type CWMusic     'Music
    ID As Long
End Type

Public Type CWMusicObj     'Music
    IsLoop As Boolean
    mc As IMediaControl
...
Public Type CWKeyState      'Button state class
    PUP As Boolean
    PDown As Boolean
...
Public Type CWKeyStateSP        'Mouse Wheel status class
    PUP As Boolean
    PDown As Boolean
    PUPMoment As Boolean
    PDownMoment As Boolean
...
Public Type CWMouseState        'Mouse state
    X As Single
    Y As Single
...
Public Type CWKeyboardState     'Keyboard class
    ESC As CWKeyState
    F1 As CWKeyState
    F2 As CWKeyState
    F3 As CWKeyState
...
Public Type CWJoystickState        'Handle
...
Public CWGameRun As Boolean 'Running status
Public CWDpp9 As D3DPRESENT_PARAMETERS 'Canvas
Public CWD3Dc9 As D3DCAPS9  'Device characteristics
Public CWWindowSwitch As Boolean 'Full screen window switch
...
Public CWFontList() As D3DXFont, CWFontNum As Long  'Text processing list
Public CWMusicList() As CWMusicObj, CWMusicNum As Long 'Music processing list
...
Public CWP_PubRollCD As D3DVECTOR   'Common fixed values for sprite maps

Enum CWDisplayModel     'Display mode constant
    CW_Windowed = 1
    CW_FullScreen = 0
End Enum

Enum CWFAlign               'Text alignment constant
    CWF_LeftAl = DT_LEFT Or DT_WORDBREAK
    CWF_RightAl = DT_RIGHT Or DT_WORDBREAK
    CWF_CenterAl = DT_CENTER Or DT_WORDBREAK
End Enum

Enum CWFBStyle              'Font weight constant
    CWF_Normal = 400
    CWF_Bold = 700
End Enum

Enum CWMPModel              'Music playback mode constant
    CWM_Resume
    CWM_Once
    CWM_Repeat
    CWM_Restart
...
Public Const CWP_SpriteConst As Long = D3DXSPRITE_DONOTMODIFY_RENDERSTATE Or D3DXSPRITE_DONOTSAVESTATE  '¾«Áé²ÎÊý³£Á¿
Public Const Pi As Single = 3.14159265358979                  'pi const
...
Enum CWColorConstants
    CWColorNone = &H0          	'Completely colorless
    CWTransparent = &HFFFFFF	'Transparent color (convenient bit operation to intercept the color part)
    
    CWBlack = &HFF000000        'Black
    CWWhite = &HFFFFFFFF        'White
    CWGrey = &HFF808080         'Gray
    CWRed = &HFFFF0000          'Red
    CWGreen = &HFF00FF00        'Green 
    CWBlue = &HFF0000FF         'Blue
    CWYellow = &HFFFFFF00       'Yellow
    CWPurple = &HFFFF00FF       'Purple
    CWCyan = &HFF00FFFF         'Cyan
    CWOrange = &HFFFF8000       'Orange
    CWkelly = &HFF80FF00        'Yellow
    CWFuchsia = &HFFFF0080      'Green
    CWViolet = &HFF8000FF       'Purple-red' Blue-purple
    CWTurquoise = &HFF00FF80    'Cyan
    CWCyanine = &HFF0080FF      'Blue
    
    CWHA_Black = &H80000000     'Translucent black
    CWHA_White = &H80FFFFFF     'Translucent white
    CWHA_Grey = &H80808080      'Translucent gray
    CWHA_Red = &H80FF0000       'Translucent red
    CWHA_Green = &H8000FF00     'Translucent green
    CWHA_Blue = &H800000FF      'Translucent blue
    CWHA_Yellow = &H80FFFF00    'Translucent yellow
    CWHA_Purple = &H80FF00FF    'Translucent purple
    CWHA_Cyan = &H8000FFFF      'Translucent cyan
    CWHA_Orange = &H80FF8000    'Orange
    CWHA_kelly = &H8080FF00     'Yellow-green
    CWHA_Fuchsia = &H80FF0080   'Fuchsia
    CWHA_Violet = &H808000FF    'Blue-violet
    CWHA_Turquoise = &H8000FF80 'Turquoise
    CWHA_Cyanine = &H800080FF   'Cyan blue
End Enum
'For other colors, please refer to the RGB color list and use the CWColorARGB 
'function to convert them yourself'*************************Common color list***************************
...
Dim I As Integer, Anti As Integer, JSorH As Long
    ...
    MsgBox "Game is already running", vbInformation, "Repeat run"
    End
    Exit Sub
    End If
timeBeginPeriod 1
...
Frm.ScaleMode = 3     'The window display area size is calculated in pixels
Frm.BorderStyle = 0
Frm.Caption = Frm.Caption
Frm.Width = ScrWidth * Zoom * 15 'Ensure that the window display area conforms to the engine resolution to be initialized to prevent graphics distortion
...
On Error GoTo CWIniEHD
    ...
    CWDpp9.PresentationInterval = D3DPRESENT_INTERVAL_ONE        'Turn on vertical sync

'    On Error Resume Next
'    Anti = 0
'    For i = 16 To 2 Step -2         'Detect anti-aliasing multiple
'    Err.Clear
...
'        End If
'    Next
'    If Anti > 0 Then CWDpp9.MultiSampleType = Anti           'Anti-aliasing

On Error GoTo CWIniEHD

    CWD3D9.GetDeviceCaps 0, D3DDEVTYPE_HAL, CWD3Dc9
    JSorH = 0
    If CWD3Dc9.DevCaps And D3DDEVCAPS_HWTRANSFORMANDLIGHT Then        'Check whether hardware vertex rendering is supported
    ...
    ' Alpha blending is better than Alpha testing, but Alpha testing directly removes opaque pixels and does not require blending operations, which can improve


    ' Alpha blending (to achieve transparency, can be fully transparent, opaque, or semi-transparent)
    ...
    ' Alpha test (can only achieve full transparency and opacity, not semi-transparency, but more efficient than Alpha blending)
    ...
    CWD3DDevice9.SetRenderState D3DRS_LIGHTING, False  'No need to use light when drawing
    CWD3DDevice9.SetRenderState D3DRS_ZWRITEENABLE, False   'No need to use Z-Buffer when drawing
    ...
    ReDim CWFontList(0)
        ...
        CWMusicNum = 0
            ...
            CWGameRun = True   'The game state is set to running

    ReDim CWJoystick(0 To joyGetNumDevs() - 1)
Exit Sub
CWIniEHD:
    MsgBox "Engine initialization failed, please make sure you have fully installed the latest version of DirectX 9.0c. If the problem still occurs, please confirm whether your graphics card is compatible", vbInformation, "DirectX"
    End
Exit Sub

CWFIniEDH:
    MsgBox "Form initialization failed. The form initialized by CoolWind engine must be an object with handle and client area.", vbInformation, "Form Error"
    End
...
Public Sub CWPresentScene()
    CWD3DDevice9.EndScene 'Scene ends
        
    DoEvents    ' Allow the system to process other information (such as obtaining keyboard and mouse status etc.) 
    CWMediaLoopRepair
    CWD3DDevice9.Present ByVal 0, ByVal 0, 0, ByVal 0   ' Refresh
        
    CWGetFPS    'Get FPS
    If CWFPS > 86 Then Sleep 10  'Reduce CPU usage when the window is completely blocked
...
Public Sub SaveScreenShot(file_name As String, Optional ByVal Format As D3DXIMAGE_FILEFORMAT = D3DXIFF_PNG) '½ØÆÁ
...
Public Sub CWResetDevice()      'Reset device¸
Dim I As Long
On Error GoTo CWResetEHD

If CWD3DDevice9.TestCooperativeLevel = D3DERR_DEVICENOTRESET Then  'Device loss detected
    ...
    CWD3DDevice9.SetRenderState D3DRS_ALPHABLENDENABLE, True
    CWD3DDevice9.SetRenderState D3DRS_SRCBLEND, D3DBLEND_SRCALPHA         'Turn on transparent color when drawing
    CWD3DDevice9.SetRenderState D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA
    ...
    CWD3DDevice9.SetRenderState D3DRS_LIGHTING, False  'No need to use light when drawing 
    CWD3DDevice9.SetRenderState D3DRS_ZWRITEENABLE, False   'No need to use Z-Buffer when drawing
    CWSprite.OnResetDevice           'Because textures and fonts have been set to system managed, they are not available or the time being
...
Exit Sub
CWResetEHD:
MsgBox "Reset device failed", vbInformation, "Device missing"
End Sub
...
On Error GoTo CWResetEHD
    ...
    CWD3DDevice9.SetRenderState D3DRS_LIGHTING, False  'No need to use light when drawing 
    CWD3DDevice9.SetRenderState D3DRS_ZWRITEENABLE, False   'No need to use Z-Buffer when drawing
    CWSprite.OnResetDevice           'Because textures and fonts have been set to system managed, they are not available for the time being
    CWSpriteSP.OnResetDevice
    CWLine.OnResetDevice
    For I = 1 To CWFontNum
      CWFontList(I).OnResetDevice
...
Exit Sub
CWResetEHD:
MsgBox "Full screen/window mode switching failed", vbInformation, "Device lost"
CWWindowSwitch = False
End Sub
...
CWLPEHD:
    MsgBox "Image not found or image format not supported", vbInformation, "Texture initialization cannot find the image or does not support the image format", vbInformation, "texture initialization failed"
    End
End Sub
...
CWLPEHD:
    MsgBox "Non-GDI+ bitmap or unsupported pixel format", vbInformation, "Texture initialization failed"
    End
End Sub
...
End Property
 ...
 'Draw the image (no scaling, no cropping, no rotation)
...
' Draw the image (scaled, not cropped, not rotated)
...
' Draw the image (no scaling, cropping, or rotation)
...
' Draw the image (scale, crop, do not rotate)
...
' Draw the image (scale, crop, rotate)
...
' Draw the picture (rotate first, then scale, can be cropped)
...
' Draw the picture (scale first, then rotate, can be cropped)
...
' Draw the picture (full version: scaling and rotation are independent, there is no order relationship, and it can be cropped)
...
, Optional ByVal RollX As Single, Optional ByVal RollY As Single, Optional ByVal RollAngle As Single, Optional ByVal HColor As CWColorConstants = CWWhite)
    ...
    With ScaleCenterCoordinate
        ...
        .Y = ZoomCY
    End With
    With RollCoordinate 'ÉèÖÃÍ¼Æ¬µÄ×ª¶¯Öá×ø±ê
        .X = RollX
        .Y = RollY
    End With
...
Public Sub CWDrawPoint(ByVal OX As Single, ByVal OY As Single, ByVal CColor As CWColorConstants)
    ...
    CWPaintPicFlush ' Submit the drawing of the sprite first, then draw the graphics (otherwise there will be problems with the hierarchy)
...
' Get the playback duration (the integer part is in seconds, the decimal part is used for fine-tuning)
...
' Whether to support 3D spatial sound effects
...
' Set the spatial position of the sound source (for 2D games, just ignore one of the Y-axis or Z-axis values)
' A negative number for X means left, a positive number for right.
' A negative number for Y indicates down, and a positive number indicates up.
' Z is negative for the back and positive for the front
' ' For 2D games like Zha Zha Hui that require forward, backward, left, and right movement, it is recommended to ignore the Z 
' axis. ' For 2D games like MapleStory that require up, down, left, and right movement, it is recommended to ignore the Y axis.
...
' PicMode is True, which means the linear dodge mode of PS (used for textures as light sources)
' PicMode is False to indicate the original lighting mode (used to fill graphics as a light source)
...
Public Sub CWKeyboardCheck()
    ...
    'CWCheckKey vbKeyMenu, CWKeyboard.Alt        ''Needs correction
    ...
    For I = LBound(CWJoystick) To UBound(CWJoystick)
        ...
        With CWJoystick(I)
            ...
            If .IsConnected Then
                ...
                Next
            Else
                ' Failed to read the handle (maybe because it is not inserted)
